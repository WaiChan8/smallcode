...
const COMPOUND_TOOLS = [
  ...
  { type: 'function', function: { name: 'tdd_begin_cycle', description: 'Start a new TDD cycle for a named test. Call this after writing the failing test, before running it. Moves the TDD state machine to the RED phase and sets the target test to track.', parameters: { type: 'object', properties: { test_name: { type: 'string', description: 'The test name or identifier to track through the Red→Green→Refactor cycle (e.g. "test_add" or "should return 404 for missing user").' } }, required: ['test_name'] } } },
  { type: 'function', function: { name: 'tdd_status', description: 'Show the current TDD phase, target test, and what action is required next. Use this to orient yourself when resuming a TDD session.', parameters: { type: 'object', properties: {}, required: [] } } },
  { type: 'function', function: { name: 'tdd_advance', description: 'Attempt to advance the TDD cycle to the next phase. In RED phase: call after writing implementation to advance to GREEN. In GREEN phase: call to enter REFACTOR. In REFACTOR phase: call after a clean full-suite run to complete the cycle. Always call run_tests first to have current results.', parameters: { type: 'object', properties: { skip_refactor: { type: 'boolean', description: 'Set to true to skip the REFACTOR phase and complete the cycle directly from GREEN. Use only for simple fixes.' } }, required: [] } } },
  { type: 'function', function: { name: 'tdd_reset', description: 'Reset the TDD state machine to idle. Use if the current cycle needs to be abandoned or restarted.', parameters: { type: 'object', properties: {}, required: [] } } },
];
